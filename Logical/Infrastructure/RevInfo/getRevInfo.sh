...
(* RevInfo.var 2.0.0 *)
VAR CONSTANT
	gRevInfo_revision : STRING[80] := '$(git describe --always --tags)';
	revision : STRING[80] := '$(git describe --always --tags)';
	gRevInfo_revisionDate : STRING[80] := '$(git show -s --date=default --pretty=format:%ci)';
	gRevInfo_revisionAuthor : STRING[80] := '$(git show -s --pretty=format:%an)';
	gRevInfo_branchName : STRING[80] := '$(git branch 2> /dev/null | sed -e '/^[^*]/d' -e 's/* \(.*\)/\1/')';
	gRevInfo_uncommittedChanges : STRING[80] := '$(git diff --shortstat)';
	gRevInfo_buildDate  : STRING[80] := '$(date +"%Y-%m-%d %H:%M:%S %z")';
	gRevInfo_buildConfiguration : STRING[80] := '$2';
	buildConfiguration : STRING[80] := '$2';
	gRevInfo_builder : STRING[80] := '$3';
	gRevInfo_versionId : STRING[80] := '$(git describe --always --tags)';
END_VAR
EOF
